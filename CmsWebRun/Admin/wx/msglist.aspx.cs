...
public partial class msglist : System.Web.UI.Page
{
    ...
    Cms.BLL.wx_msg bllorder = new Cms.BLL.wx_msg();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            string where = "select * from wx_msg  order by orderNumber asc";
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        foreach (RepeaterItem item in rptList.Items)
        {
            ...
            if (check.Checked)
            {
                ...
                //删除文档的同时删除静态文档
                adminUser.AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), bllorder.GetModel(id).info); //记录日志
                bllorder.Delete(id);
                

            }
        }
        JscriptMsg("删除信息成功！", "msglist.aspx?parentId=" + strparentId, "Success");
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            string where = "select * from wx_msg  where orderNumber like '%" + skeyword + "%'  order by id desc";
            this.RepeaterDataBind(where.ToString());
        }
    }
}