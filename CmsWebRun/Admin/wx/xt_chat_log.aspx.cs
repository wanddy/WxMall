...
public partial class Admin_wx_xt_chat_log : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            Cms.BLL.XT_ChatLog bll = new Cms.BLL.XT_ChatLog();
            string where = "select * from XT_ChatLog  order by Chatid desc";
    ...
    protected string CombSqlTxt(string _keywords)
    {
        ...
        {
            strTemp.Append(" and (FromUserName like  '%" + _keywords + "%' or ToUserNa like '%" + _keywords + "%')");
        }
    ...
    public void RepeaterDataBind(string whereStr)
    {
        ...
        ds = new DataSet();
        dr.Fill(ds, AspNetPager1.PageSize * (AspNetPager1.CurrentPageIndex - 1), AspNetPager1.PageSize, "XT_ChatLog");
        this.rptList.DataSource = ds.Tables["XT_ChatLog"];
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("xt_chat_log.aspx", "keywords={0}", txtKeywords.Text));
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        int sucCount = Cms.DBUtility.DbHelperSQL.ExecuteSql("delete from XT_ChatLog where DATEDIFF(day, CreateTime, getdate()) > 7");//修改
        adminUser.AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), "删除管理日志" + sucCount + "条"); //记录日志
        JscriptMsg("删除日志" + sucCount + "条", Utils.CombUrlTxt("xt_chat_log.aspx", "keywords={0}", this.keywords), "Success");
    }
