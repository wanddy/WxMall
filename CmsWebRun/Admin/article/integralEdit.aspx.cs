...
public partial class Admin_article_integralEdit : System.Web.UI.Page
{
    ...
    {
        AjaxPro.Utility.RegisterTypeForAjax(typeof(Admin_article_integralEdit));//注册Ajax可调用的类的名称
    ...
    public void DataBind(int articleId)
    {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            stock.Text = dt.Rows[0]["integral"].ToString();
            productId.Value = dt.Rows[0]["id"].ToString();
            type.SelectedValue = dt.Rows[0]["type"].ToString();
            style.Text = dt.Rows[0]["style"].ToString();
            kim_joong.Text = dt.Rows[0]["kim_joong"].ToString();
            inch_number.Text = dt.Rows[0]["inch_number"].ToString();
            gold_price.Text = Convert.ToDecimal(dt.Rows[0]["gold_price"] == null ? "0.00" : dt.Rows[0]["gold_price"]).ToString("0.00");
            labor_charges.Text = Convert.ToDecimal(dt.Rows[0]["labor_charges"] == "" ? "0.00" : dt.Rows[0]["labor_charges"]).ToString("0.00");
            color.SelectedValue = dt.Rows[0]["color"].ToString();
            clarity.SelectedValue = dt.Rows[0]["clarity"].ToString();
            cut.SelectedValue = dt.Rows[0]["cut"].ToString();
            this.diamond_shape.SelectedValue = dt.Rows[0]["diamond_shape"].ToString();
            main_stone_weight.Text = dt.Rows[0]["main_stone_weight"].ToString();
            main_stone_number.Text = dt.Rows[0]["main_stone_number"].ToString();
            total_weight.Text = dt.Rows[0]["total_weight"].ToString();
            sales_labor_charge.Text = Convert.ToDecimal(dt.Rows[0]["sales_labor_charge"] == "" ? "0.00" : dt.Rows[0]["sales_labor_charge"]).ToString("0.00");
            accessories_price.Text = Convert.ToDecimal(dt.Rows[0]["accessories_price"] == "" ? "0.00" : dt.Rows[0]["accessories_price"]).ToString("0.00");
            product_category.Text = dt.Rows[0]["product_category"].ToString();
            auxiliary_stone_weight.Text = dt.Rows[0]["auxiliary_stone_weight"].ToString();
            side_stone_number.Text = dt.Rows[0]["side_stone_number"].ToString();
            certificate_no.Text = dt.Rows[0]["certificate_no"].ToString();
            userscore.Text = dt.Rows[0]["userscore"].ToString();
            ...
            foreach (string i in sArray)
            {
                ...
                for (int j = 0; j < finger_ring.Items.Count; j++)
                {
                    if (i.ToString() == finger_ring.Items[j].Value.ToString())
                    {
                        finger_ring.Items[j].Selected = true;
                    }
                }
            }
    ...
    public void DataAdd()
    {
        ...
        if (result > 0)
        {
            ...
            for (int i = 0; i < finger_ring.Items.Count; i++)
            {
                if (finger_ring.Items[i].Selected == true)
                {
                    sbuBuilder.Append(finger_ring.Items[i].Value + ",");
                }
            }
            ...
            JscriptMsg("添加信息成功！", "integralList.aspx?classid=" + this.parentId.SelectedValue, "Success");
        }
        else
        {
            JscriptMsg("添加信息失败！", "integralEdit.aspx?action=add&classid=" + this.parentId.SelectedValue, "Error");
        }
    }
    #endregion
    ...
    public void DataUpdate(int articleId)
    {
        ...
        if (bllarticle.Update(modelarticle))
        {
            ...
            JscriptMsg("修改信息成功！", "integralList.aspx?classid=" + this.parentId.SelectedValue, "Success");
        }
        else
        {
            JscriptMsg("修改信息失败！", "integralEdit.aspx?action=edit&classid=" + this.parentId.SelectedValue, "Error");
        }
    }
    #endregion
