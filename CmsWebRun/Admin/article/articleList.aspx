<%@ Page Language="C#" AutoEventWireup="true" Inherits="Admin_article_articleList" Codebehind="articleList.aspx.cs" %>
...
<div class="location">
  ...
  <i class="arrow"></i>
  <span><%=new Cms.BLL.C_Column().GetModel(Convert.ToInt32(Request["classid"].ToString())).className.ToString()%></span>
  <i class="arrow"></i>
  <span>内容列表</span>
</div>
<!--/导航栏-->
...
<ItemTemplate>
  <tr>
    <td align="center"><asp:CheckBox ID="Check_Select" CssClass="checkall" runat="server" style="vertical-align:middle;" /><asp:HiddenField ID="Fielddocid" Value='<%#Eval("articleId")%>' runat="server" /></td>
    <td><a href="articleEdit.aspx?action=edit&articleId=<%#Eval("articleId")%>&classid=<%#Eval("parentId")%>"><%#Eval("title")%></a></td>
    <td><%# new Cms.BLL.C_Column().GetModel(Convert.ToInt32(Eval("parentId"))).className%></td>
    <td><%#string.Format("{0:g}", Eval("updateTime"))%></td>
    <td><asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("ordernumber")%>' CssClass="sort" onkeydown="return checkNumber(event);" /></td>
    <td>
      <div class="btn-tools">
        ...
        <asp:LinkButton ID="lbtnIsTop" CommandName="lbtnIsTop" runat="server" CssClass='<%# Convert.ToInt32(Eval("isTop")) == 1 ? "top selected" : "top"%>' ToolTip='<%# Convert.ToInt32(Eval("isTop")) == 1 ? "取消置顶" : "设置置顶"%>' />
        <asp:LinkButton ID="lbtnIsRed" CommandName="lbtnIsRed" runat="server" CssClass='<%# Convert.ToInt32(Eval("isRecommend")) == 1 ? "red selected" : "red"%>' ToolTip='<%# Convert.ToInt32(Eval("isRecommend")) == 1 ? "取消推荐" : "设置推荐"%>' />
        <asp:LinkButton ID="lbtnIsHot" CommandName="lbtnIsHot" runat="server" CssClass='<%# Convert.ToInt32(Eval("isHot")) == 1 ? "hot selected" : "hot"%>' ToolTip='<%# Convert.ToInt32(Eval("isHot")) == 1 ? "取消热门" : "设置热门"%>' />
    ...
    <td align="center">
    <asp:LinkButton ID="lbedit" runat="server" CommandArgument='<%#Eval("articleId")%>' CommandName='<%#Eval("parentId")%>' oncommand="lbedit_Command">修改</asp:LinkButton>
    </td>
  </tr>
</ItemTemplate>
<FooterTemplate>
...
<ItemTemplate>
    <li>
      <div class="details<%#Eval("photoUrl").ToString() != "" ? "" : " nopic"%>">
        <div class="check"><asp:CheckBox ID="Check_Select" CssClass="checkall" runat="server" /><asp:HiddenField ID="Fielddocid" Value='<%#Eval("articleId")%>' runat="server" /></div>
        <%#Eval("photoUrl").ToString() != "" ? "<div class=\"pic\"><img src=\"../skin/default/loadimg.gif\" data-original=\"" + Eval("photoUrl") + "\" /></div><i class=\"absbg\"></i>" : ""%>
        <h1><span><a href="articleEdit.aspx?action=edit&articleId=<%#Eval("articleId")%>&classid=<%#Eval("parentId")%>"><%#Eval("title")%></a></span></h1>
        <div class="remark">
          <%#Eval("content").ToString() == "" ? "暂无内容摘要说明..." : Cms.Web.ToAspx.getLength(Cms.Web.ToAspx.GetToHtml(Eval("content").ToString()), 100)%>
        </div>
        <div class="tools">
          <asp:LinkButton ID="lbtnIsMsg" CommandName="lbtnIsMsg" runat="server" CssClass='<%# Convert.ToInt32(Eval("is_msg")) == 1 ? "msg selected" : "msg"%>' ToolTip='<%# Convert.ToInt32(Eval("is_msg")) == 1 ? "取消评论" : "设置评论"%>' />
          <asp:LinkButton ID="lbtnIsTop" CommandName="lbtnIsTop" runat="server" CssClass='<%# Convert.ToInt32(Eval("isTop")) == 1 ? "top selected" : "top"%>' ToolTip='<%# Convert.ToInt32(Eval("isTop")) == 1 ? "取消置顶" : "设置置顶"%>' />
          ...
          <asp:LinkButton ID="lbtnIsHot" CommandName="lbtnIsHot" runat="server" CssClass='<%# Convert.ToInt32(Eval("isHot")) == 1 ? "hot selected" : "hot"%>' ToolTip='<%# Convert.ToInt32(Eval("isHot")) == 1 ? "取消热门" : "设置热门"%>' />
          <asp:LinkButton ID="lbtnIsSlide" CommandName="lbtnIsSlide" runat="server" CssClass='<%# Convert.ToInt32(Eval("is_slide")) == 1 ? "pic selected" : "pic"%>' ToolTip='<%# Convert.ToInt32(Eval("is_slide")) == 1 ? "取消幻灯片" : "设置幻灯片"%>' />
         <asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("ordernumber")%>' CssClass="sort" onkeydown="return checkNumber(event);" />
        </div>
        <div class="foot">
          <p class="time"><%#string.Format("{0:yyyy-MM-dd HH:mm:ss}", Eval("updateTime"))%></p>

           <asp:LinkButton ID="lbedits" runat="server" CommandArgument='<%#Eval("articleId")%>' CommandName='<%#Eval("parentId")%>' CssClass="edit" title="编辑"  oncommand="lbedit_Command">编辑</asp:LinkButton>
...
<div class="pagelist">
  ...
  <webdiyer:AspNetPager ID="AspNetPager1" runat="server" HorizontalAlign="Center" FirstPageText="首页"
          ...
          ShowBoxThreshold="2" InputBoxClass="text2"
                        TextAfterInputBox="" 
