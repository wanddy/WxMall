...
public partial class Admin_article_articleEdit : System.Web.UI.Page
{
    ...
    {
        AjaxPro.Utility.RegisterTypeForAjax(typeof(Admin_article_articleEdit));//注册Ajax可调用的类的名称
        int articleId = Convert.ToInt32(this.Request.QueryString["articleId"] ?? "0");//文章ID
        if (articleId>0&&!new Cms.BLL.C_article().Exists(articleId))
        {
        ...
        if (!Page.IsPostBack)
        {
            ...
            //this.Bind_type();//绑定产品分类
            //Bind_bardtype();//绑定品牌分类
            //this.Bind_colortype();//绑定颜色分类
    ...
    public void DataAdd()
    {
        ...
        if (result > 0)
        {
            adminUser.AddAdminLog(DTEnums.ActionEnum.Add.ToString(), "添加" + channel_name + ":" + modelarticle.title); //记录日志
            JscriptMsg("添加信息成功！", "articleList.aspx?classid=" + this.parentId.SelectedValue, "Success");
        }
        else
        {
            JscriptMsg("添加信息失败！", "articleEdit.aspx?action=add&classid=" + this.parentId.SelectedValue, "Error");
        }
    }
    #endregion
    ...
    public void DataUpdate(int articleId)
    {
        ...
        {
            adminUser.AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "修改" + channel_name + ":" + modelarticle.title); //记录日志
            JscriptMsg("修改信息成功！", "articleList.aspx?classid=" + this.parentId.SelectedValue, "Success");
        }
        else
        {
            JscriptMsg("修改信息失败！", "articleEdit.aspx?action=edit&classid=" + this.parentId.SelectedValue, "Error");
        }
    }
    #endregion
    ...
    public void Bind_type()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                productype.Items.Add(item);
                ChileNodeBindType(dr, productype, 2);
            }
        }
    }

    private void ChileNodeBindType(DataRow drr, ListBox parentId, int m)
    {
        Cms.BLL.C_article_category bllcolumn = new Cms.BLL.C_article_category();
        Cms.Model.C_article_category modelcolumn = new Cms.Model.C_article_category();
        ...
        if (dss.Tables[0].Rows.Count > 0)
        {
            ...
            for (int k = 0; k < dss.Tables[0].Rows.Count; k++)
            {
                ...
                ChileNodeBindType(dro, parentId, m + 5);
            }
        }
    }

    protected void productype_SelectedIndexChanged(object sender, EventArgs e)
    {
        ListItem myitem = new ListItem();
        myitem.Value = productype.SelectedValue;
        string zifu = productype.Items[productype.SelectedIndex].Text;
        string result = zifu.Substring(zifu.IndexOf('├') + 1);
        myitem.Text = result;

        if (!productypechose.Items.Contains(myitem))
        {
            Cms.BLL.C_article_category bll = new Cms.BLL.C_article_category();
            int id = Convert.ToInt32(myitem.Value);
            DataSet ds = Cms.DBUtility.DbHelperSQL.Query("select max(class_layer) as class_layermax from C_article_category where channel_id=1");
            DataSet ds1 = bll.GetList("id=" + id);
            int class_layermax = Convert.ToInt32(ds.Tables[0].Rows[0]["class_layermax"].ToString());
            int class_layer = Convert.ToInt32(ds1.Tables[0].Rows[0]["class_layer"].ToString());
            if (class_layer == class_layermax)
            {
                productypechose.Items.Add(myitem);
            }
            else
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), " ", "<script>alert('请勿重复添加！')</script>");
            }
        }
        else
        {
    ...
    protected void productypechose_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (productypechose.SelectedIndex != -1)
        {
            productypechose.Items.RemoveAt(productypechose.SelectedIndex);
        }
        else
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), " ", "<script>alert('请选中右边的分类！')</script>");
        }
    ...
    #region 绑定品牌分类=======================================
    public void Bind_bardtype()
    {
        bardtype.Items.Clear();
        Cms.BLL.C_article_category bllcolumn = new Cms.BLL.C_article_category();
        Cms.Model.C_article_category modelcolumn = new Cms.Model.C_article_category();
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                bardtype.Items.Add(item);
                ChileNodeBindbard(dr, bardtype, 2);
            }
        }
    }

    private void ChileNodeBindbard(DataRow drr, ListBox parentId, int m)
    {
        Cms.BLL.C_article_category bllcolumn = new Cms.BLL.C_article_category();
        Cms.Model.C_article_category modelcolumn = new Cms.Model.C_article_category();
    ...
    protected void bardtype_SelectedIndexChanged(object sender, EventArgs e)
    {
        ListItem myitem = new ListItem();
        myitem.Value = bardtype.SelectedValue;
        string zifu = bardtype.Items[bardtype.SelectedIndex].Text;
        string result = zifu.Substring(zifu.IndexOf('├') + 1);
        myitem.Text = result;

        if (!bardtypechose.Items.Contains(myitem))
        {
            Cms.BLL.C_article_category bll = new Cms.BLL.C_article_category();
            int id = Convert.ToInt32(myitem.Value);
            DataSet ds = Cms.DBUtility.DbHelperSQL.Query("select max(class_layer) as class_layermax from C_article_category where channel_id=4");
            DataSet ds1 = bll.GetList("id=" + id);
            int class_layermax = Convert.ToInt32(ds.Tables[0].Rows[0]["class_layermax"].ToString());
            int class_layer = Convert.ToInt32(ds1.Tables[0].Rows[0]["class_layer"].ToString());
            if (class_layer == class_layermax)
            {
                if (bardtypechose.Items.Count < 1)
                {
                    bardtypechose.Items.Add(myitem);
                }
                else
                {
                    Page.ClientScript.RegisterStartupScript(this.GetType(), " ", "<script>alert('请勿重复添加！')</script>");
    ...
    protected void bardtypechose_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (bardtypechose.SelectedIndex != -1)
        {
            bardtypechose.Items.RemoveAt(bardtypechose.SelectedIndex);
        }
        else
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), " ", "<script>alert('请选中右边的分类！')</script>");
    ...
    public void Bind_colortype()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                colortype.Items.Add(item);
                ChileNodeBindcolorType(dr, colortype, 2);
            }
        }
    }

    private void ChileNodeBindcolorType(DataRow drr, ListBox parentId, int m)
    {
        Cms.BLL.C_article_category bllcolumn = new Cms.BLL.C_article_category();
        Cms.Model.C_article_category modelcolumn = new Cms.Model.C_article_category();
        ...
        if (dss.Tables[0].Rows.Count > 0)
        {
            ...
            for (int k = 0; k < dss.Tables[0].Rows.Count; k++)
            {
                ...
                ChileNodeBindcolorType(dro, parentId, m + 5);
            }
        }
    }
    ...
    {
        ListItem myitem = new ListItem();
        myitem.Value = colortype.SelectedValue;
        string zifu = colortype.Items[colortype.SelectedIndex].Text;
        bool b = zifu.IndexOf('├') > 0;
        if (b == true)
        {
            string result = zifu.Substring(zifu.IndexOf('├') + 1);
            myitem.Text = result;
        ...
        {
            myitem.Text = colortype.Items[colortype.SelectedIndex].Text;
        }
        if (!colortypechose.Items.Contains(myitem))
        {
            Cms.BLL.C_article_category bll = new Cms.BLL.C_article_category();
            int id = Convert.ToInt32(myitem.Value);
            DataSet ds = Cms.DBUtility.DbHelperSQL.Query("select max(class_layer) as class_layermax from C_article_category where channel_id=2");
            DataSet ds1 = bll.GetList("id=" + id);
            int class_layermax = Convert.ToInt32(ds.Tables[0].Rows[0]["class_layermax"].ToString());
            int class_layer = Convert.ToInt32(ds1.Tables[0].Rows[0]["class_layer"].ToString());
            if (class_layer == class_layermax)
            {
                colortypechose.Items.Add(myitem);
            }
            else
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), " ", "<script>alert('请勿重复添加！')</script>");
    ...
    protected void colortypechose_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (colortypechose.SelectedIndex != -1)
        {
            colortypechose.Items.RemoveAt(colortypechose.SelectedIndex);
        }
        else
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), " ", "<script>alert('请选中右边的分类！')</script>");
