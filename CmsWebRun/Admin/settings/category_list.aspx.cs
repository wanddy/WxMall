...
using Cms.DBUtility;
using Cms.Common;
public partial class Admin_settings_category_list : System.Web.UI.Page
{
    ...
    public void updateSate(int id, string state)
    {
        ...
        JscriptMsg("设置成功！", "category_list.aspx?id=" + channel_id, "Success");
    }
    #endregion
    ...
    {
        id = Convert.ToInt32(this.Request.QueryString["id"] ?? "0");//类别ID
        Response.Redirect("category_edit.aspx?action=add&channel_id=" + id);
    }
    protected void lbedit_Command(object sender, CommandEventArgs e)
    {
        ...
        Response.Redirect("category_edit.aspx?action=edit&channel_id=" + pid + "&id=" + cid + "");
    }
    protected void lbadd_Command(object sender, CommandEventArgs e)
    {
        string cid = e.CommandArgument.ToString();
        string pid = e.CommandName.ToString();
        Response.Redirect("category_edit.aspx?action=add&channel_id=" + pid + "&id=" + cid + "");
    }
}