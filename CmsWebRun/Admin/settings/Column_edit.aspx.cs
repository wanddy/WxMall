...
using System.Data;
using Cms.Common;
public partial class Admin_settings_Column_edit : System.Web.UI.Page
{
    ...
    public void DropList_Bind()
    {
        ...
        parentId.Items.Add(new ListItem("作为一级分类", "26"));
        Cms.BLL.C_Column bllcolumn = new Cms.BLL.C_Column();
        Cms.Model.C_Column modelcolumn = new Cms.Model.C_Column();
        DataSet ds = bllcolumn.GetList("parentId=26 order by orderNumber desc");
        if (ds.Tables[0].Rows.Count > 0)
        {
    ...
    public void DataAdd()
    {
        ...
        if (result > 0)
        {
            ...
            JscriptMsg("添加栏目信息成功！", "Column_list.aspx", "Success");
        }
        else
        {
            JscriptMsg("添加栏目信息失败！", "Column_edit.aspx?action=add", "Error");
        }
    }
    #endregion
    ...
    {
        bool blEdit = adminUser.setpurview(this.className.Text.Trim(), "Edit");
        if (!blEdit)
        {
            JscriptMsg("修改失败，您没有修改权限！", "Column_list.aspx", "Error");
            return;
        }
        ...
        if (bllcolumn.Update(modelcolumn))
        {
            ...
            JscriptMsg("修改栏目信息成功！", "Column_list.aspx", "Success");
        }
        else
        {
            JscriptMsg("修改栏目信息失败！", "Column_edit.aspx?action=edit&classid=" + classid, "Error");
        }
    }
    #endregion
