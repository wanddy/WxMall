...
using Cms.DBUtility;
using Cms.Common;
public partial class Admin_manager_manager_list : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            string where = "select * from C_admin where role_type>=" + Convert.ToInt32(Application["adminType"]) + "  order by id desc ";
            this.AspNetPager1.AlwaysShow = true;
            this.AspNetPager1.PageSize = 10;
            this.AspNetPager1.RecordCount = blladmin.GetRecordCount("");
            this.RepeaterDataBind(where);


            bool bladd = adminUser.setpurview("管理员列表", "Add");
          
            bool blDelete = adminUser.setpurview("管理员列表", "Delete");
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        foreach (RepeaterItem item in rptList.Items)
        {
            ...
            if (check.Checked)
            {
                ...
                //删除文档的同时删除静态文档
                blladmin.Delete(id);
                adminUser.AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), blladmin.GetModel(id).user_name); //记录日志
            }
        }
        JscriptMsg("删除信息成功！", "manager_list.aspx", "Success");
    }
    #endregion
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        string whereStr = "select * from C_admin where name like '%" + Keywords + "%' order by id desc";
    ...
    {
        Response.Redirect("manager_edit.aspx?action=add");
    }
    protected void lbedit_Command(object sender, CommandEventArgs e)
    {
        string cid = e.CommandArgument.ToString();
        Response.Redirect("manager_edit.aspx?action=edit&id=" + cid + "");
    }
    ...
    protected void rptList_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            ...
            bool blEdit = adminUser.setpurview("管理员列表", "Edit");
